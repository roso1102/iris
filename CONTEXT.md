...
- Vertex AI API quota caps (set in Console, Aug 2026): `generate_content_requests_per_minute` = 15, `online_prediction_requests_per_minute` = 60. If ingestion starts returning `429 RESOURCE_EXHAUSTED`, check these caps first — they're per-minute hard limits enforced server-side, not budget-driven.
