...
- **Phase:** Phase 0.0 **complete**. Phase 1.0 deployed and verified on Cloud Run (revision `00052-md6`, image `sha256:8183e6d0`). Full pipeline live: Pub/Sub → preflight → Docling layout → 4-signal VLM Router (gemini-2.5-flash, us-central1) → chunker → text-embedding-004 → Qdrant write. Parallel page-level dispatch verified (2 pages processed concurrently, 5× projected for 35 pages). 38/38 unit tests green. Next: run full integration test suite (1-A through 1-I).
- **Implemented:** `ModelProvider` abstract class, `VertexAIProvider`, `MockModelProvider`, `factory.py` (768-d Vertex AI verified live). Phase 0.0: 56 Terraform resources (VPC `iris-vpc` + `iris-subnet` 10.0.0.0/20 + VPC connector on 10.0.16.0/28 + Cloud Router/NAT + Firestore VPC peering + Qdrant PSC address + qdrant-api firewall rule + Qdrant GCE VM `qdrant-1` e2-small in `asia-south1-b`). All 19 APIs enabled. `iris-raw-pdfs` bucket, ingestion topic + DLQ topic + DLQ pull subscription, main subscription (direct push to Cloud Run, ack=600s, retry 30-600s, max_delivery=5, DLQ→iris-ingestion-dlq). Billing kill switch: ₹500 monthly cap. Services: `ingestion-worker` live at `asia-south1` (2 CPU, 8 GiB, 900s timeout, max-instances=10, concurrency=1, VPC connector, no public auth). Phase 1.0 code: `services/common/ingestion/` (preflight, Docling parser, VLM Router with VLM fallback + _crop_bbox fix, chunker, QdrantChunkStore idempotent, FitzPageRenderer, PDF splitter, doc hash cache), worker `app.py` (POST / per-page, POST /ingest fan-out, GET /status/{doc_id}, GET /memory QA view, GET /healthz). Dockerfile: `TORCH_COMPILE_DISABLE=1`, libgl1 + libxcb1 + g++ system deps, Docling model warm at build, vertexai SDK. Vertex AI: `gemini-2.5-flash` (vision), `gemini-2.5-flash-lite` (lite), `text-embedding-004` (768-d), all in `us-central1`. 38 unit tests green.
