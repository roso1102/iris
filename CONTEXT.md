...
- **Phase:** Phase 0.0 and 1.0 **complete**. **Phase 2.0 core plumbing complete** — full retrieval pipeline: RRF fusion, diversity dedup, async SearchOrchestrator, FastAPI retrieval_api (`/search`, cascading delete), v2 Qdrant collection, Firestore session state, CI deploy parity, and strict tenant_id filtering. 72/72 unit tests green. **Deferred/Incomplete in Phase 2:** Task 2.5a (BM25 TF-IDF tokenization is currently weak `hash(term)` and needs a `rank_bm25` or Qdrant sparse index upgrade), VLM table validation, and deep search reranking.
- **Implemented:** `ModelProvider` abstract class, `VertexAIProvider`. Phase 0.0: 56 Terraform resources. Phase 1.0: Ingestion pipeline (preflight, Docling parser, 4-signal VLM Router with `do_ocr=False` fix and page-level Signal 4, chunker, QdrantChunkStore, PDF splitter, doc hash cache), worker on Cloud Run. **Phase 2.0:** `services/common/retrieval/` (rrf, diversity, search orchestrator). `services/common/ingestion/store.py` upgraded to `iris_chunks_v2` with `tenant_id` mandatory in `get_by_ids()`. `services/retrieval_api/app.py` rewritten as FastAPI with cascading deletes to GCS/Firestore. Ingestion progress moved to Firestore to survive cold starts. CI deployment env vars matched to `deploy.sh`.
- **Next up:** Phase 3.0 Task 3.5 — FastEmbed BM25 sparse retrieval upgrade (replaces mmh3/TF), gated behind local tests before the single GCP cycle.
